...
class HuffmanCoding:
    ...
    def __buildCodesHelper(self,root,curr_bit):
        if root is None:
            return
        if root.value is not None:
            ...
            return
        self.__buildCodesHelper(root.left,curr_bit+"0")
        self.__buildCodesHelper(root.right,curr_bit+"1")
    ...
    def compress(self):
        ...
        #construct the codes from binary tree
            self.__buildCodes()
        #creating encoded text using codes
    ...
    def decompress(self,input_path):
        filename, file_exten = os.path.splitext(self.path)
